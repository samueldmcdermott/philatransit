...
# Server-side detection of trolleys entering the underground tunnel.
...
def _process_tunnel_ghosts(transit_routes):
...
def _poll_septa_data():
    ...
    while True:
        ...
        try:
            with _transit_cache_lock:
                _routes_snap = dict(_transit_cache["routes"])
            _process_tunnel_ghosts(_routes_snap)
        except Exception as e:
...
# ── Tunnel ghosts endpoint ────────────────────────────────────
@app.route("/api/ghosts")
def get_ghosts():
    with _ghost_lock:
        result = [{**g, 'vid': vid} for vid, g in _ghosts.items()]
    return jsonify(result)
